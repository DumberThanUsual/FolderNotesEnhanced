...
var getNoteElement = function(dataPath) {
    if (dataPath) {
        ...
        // pathtoprojectfolder/projectname/
 ...
 */
var createNoteElement = function(noteKey, noteVal, pathKey) {
    let noteElement;
    ...
    if (noteVal instanceof Array) {
        for (var noteEntry in noteVal) {
            ...
            noteEntryElement.style.color = json[pathKey]["color"] || "yellow";
  ...
  toggle() {
    if (!this.notesVisible) {
      for (var i = 0; i < atom.project.getPaths().length; i++){
        const file = folderPaths[i] + '/folderNotesEnhanced.config.json';
