...
/**
  * Walk the json tree
  */
var walkJSON = function(json, constructedPath, rootPath) {
  var dataPath = rootPath + constructedPath;
  var dataPathAlt = dataPath.replace(/\//g, '\\\\');

  if (document.querySelectorAll('[data-path="' + dataPath + '"], [data-path="' + dataPathAlt + '"]').length > 0) {

    let noteEntryElements = document.querySelectorAll('[data-path="' + dataPath + '"], [data-path="' + dataPathAlt + '"]');
    for (var i = 0; i < noteEntryElements.length; i++) {
      noteEntryElements[i].appendChild(createNoteElement(constructedPath, json["notes"], json["color"]));
    }
  }
  ...
  for (let subElement in json["subElements"]) {
    ...
    walkJSON(json["subElements"][subElement], nextPath, rootPath);
  }
}
...
var renderAttributes = function(json) {
    for (var projectRoot in json) {
        ...
        console.log(json);
        walkJSON(json[projectRoot], "", projectRoot);
    }
    return false;
};
...
var destroyNotes = function() {
  var elementList = document.querySelectorAll('ul.folderNotesEnhanced');
